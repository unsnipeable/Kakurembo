...
public class PlaceHolderAPIUtil extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, @NotNull String params) {
        ...
        if (params.equalsIgnoreCase("progress2")) {
            return "&8[" + getFormatedXpBar(plugin.getGame().getGamePlayer(player).getXp()) + "&8]";
        }
        return null;
    }

    public String getFormatedXpBar(int i) {
        int filled = Math.min(i / 100, 10);
        int empty = 10 - filled;
