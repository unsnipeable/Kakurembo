...
public class PlaceHolderAPIUtil extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, @NotNull String params) {
        ...
        if (params.equalsIgnoreCase("pTime")) {
            return (double)((Math.max(plugin.getGame().getGamePlayer(player).getParkourTime(), 0))/20) + "s";
        }
        if (params.equalsIgnoreCase("pTimeLap")) {
            return (double)((Math.max(plugin.getGame().getGamePlayer(player).getParkourTime2(), 0))/20) + "s";
        }
