...
public class GameListener implements Listener {
        ...
        if (gamePlayer != null && !HideAndSeek.getInstance().getGame().isStarted()) {
            ...
            if (block.getType() == Material.LIGHT_WEIGHTED_PRESSURE_PLATE) {
                if (block2Above.getType() == Material.WHITE_WOOL) {
                    ...
                    Objects.requireNonNull(Manager.getDataManager("ParkourDataManager")).addPlayerInfoInteger(player.getUniqueId().toString(), gamePlayer.getParkourTime(), DataEnum.DataManagerType.BETTER);
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        for (Menu menu : new ArrayList<>(Registers.menus)) {
            if (!player.getOpenInventory().title().equals(menu.getTitle(player))) return;
            Map<Integer, Button> buttons = new HashMap<>(menu.getButtons(player));
